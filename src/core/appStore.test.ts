...
describe('parseAppStore', () => {
  test('JSON corrupto ⇒ defaults', () => {
    ...
    expect(d.filterTerm).toBe('sample')
  })
...
describe('recordSelection', () => {
  test('incrementa el contador y actualiza last', () => {
    ...
    d = recordSelection(d, 'com.sample.oda.qa')
    d = recordSelection(d, 'com.sample.oda.qa')
    d = recordSelection(d, 'com.android.chrome')
    expect(d.usage['com.sample.oda.qa']).toBe(2)
...
describe('rankPackages', () => {
  test('ordena por uso descendente y desempata alfabético', () => {
    const installed = ['com.zzz.app', 'com.aaa.app', 'com.sample.oda.qa', 'com.mid.app']
    const usage = { 'com.sample.oda.qa': 5, 'com.mid.app': 2, 'com.desinstalada.app': 9 }
...
describe('AppStore (persistencia)', () => {
  ...
  test('archivo inexistente ⇒ defaults; select persiste y recarga', () => {
    ...
    store.select('com.sample.oda.qa')
    store.select('com.sample.oda.qa')

    const reloaded = new AppStore(path, legacy)
    expect(reloaded.data.last).toBe('com.sample.oda.qa')
    expect(reloaded.data.usage['com.sample.oda.qa']).toBe(2)
    // el JSON en disco es legible/editable a mano (filterTerm configurable)
    const onDisk = JSON.parse(readFileSync(path, 'utf8'))
    expect(onDisk.filterTerm).toBe('sample')
  })
  ...
  test('migra silenciosamente del apps.json viejo a config.json', () => {
    ...
    writeFileSync(
      legacy,
      JSON.stringify({ last: 'com.sample.oda.qa', usage: { 'com.sample.oda.qa': 4 } }),
    )
    const store = new AppStore(cfg, legacy)
    expect(store.data.last).toBe('com.sample.oda.qa')
    ...
    expect(reloaded.data.usage['com.sample.oda.qa']).toBe(5)
  })
  ...
  test('set() aplica config validada y rechaza basura', () => {
    ...
    expect(store.data.last).toBe('com.sample.oda.qa') // select previo se preserva
  ...
  test('fpsTarget (ticket 025): default 30 y migración de configs viejas sin el campo', () => {
    ...
    const old = parseAppStore(JSON.stringify({ last: 'com.sample.oda.qa', intervalMs: 2000 }))
    expect(old.fpsTarget).toBe(30)
    expect(old.last).toBe('com.sample.oda.qa')
