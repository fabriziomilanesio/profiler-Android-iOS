...
/** Venv usado por instalaciones anteriores, antes de renombrar el profiler genérico. */
export function legacyManagedVenvPython(home: string = homedir(), os: string = platform()): string {
 ...
 * que Microsoft Store mueva o quite el Python base. Además, la edición genérica cambió
 * la carpeta gestionada; el venv anterior sigue siendo válido y permite una migración
 * sin obligar al usuario a reinstalar antes de volver a ver su iPhone.
 */
export function pythonCandidates(
  ...
  if (explicit !== undefined && explicit !== '') return [explicit]
  const candidates = [managedVenvPython(home, os), legacyManagedVenvPython(home, os)].filter(exists)
