...
describe('pythonCandidates', () => {
  test('prueba el venv genérico, el legado y por último el Python del sistema', () => {
    expect(pythonCandidates(undefined, () => true, 'C:\\Users\\x', 'win32')).toEqual([
      'C:\\Users\\x\\.sample-profiler\\pmd3-venv\\Scripts\\python.exe',
      'C:\\Users\\x\\.evermore-profiler\\pmd3-venv\\Scripts\\python.exe',
      'python',
    ])
  })

  test('si sólo existe el venv legado, lo conserva antes del fallback del sistema', () => {
    const legacy = legacyManagedVenvPython('C:\\Users\\x', 'win32')
    expect(pythonCandidates(undefined, (path) => path === legacy, 'C:\\Users\\x', 'win32')).toEqual(
      [legacy, 'python'],
    )
  })
