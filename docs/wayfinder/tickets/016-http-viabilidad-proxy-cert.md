...
`com.evermore.oda.qa` (build Unity) es interceptable. Es el riesgo #1: si el build no
...
1. **Proxy del sistema:** setear proxy global por adb
   ...
   este es el punto que puede matar el enfoque. Anotar qué stack de red usa evermore.
...
pinning sí/no) y, si algún eslabón falla, qué hay que cambiar en el build QA de evermore.
