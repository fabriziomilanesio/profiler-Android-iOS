...
title: Rename — de "Mobile Profiler" a "Mobile Profiler"
status: closed
assignee: claude
blocked-by: [038]
---

# 042 — Rename a Mobile Profiler
...
- Repo publicado en GitHub: `Generic/sample-android-profiler` (privado). Cambiar el
  nombre deja un redirect, pero hay que avisar a quien tenga el remoto clonado.
- `package.json` (`@generic/sample-android-profiler`), nombre del binario
...
- La carpeta de sesiones `~/.sample-profiler/` **ya es agnóstica** — no se toca, y
...
## Entregado (2026-08-10) — "Sample **Mobile** Profiler"

Se eligió **Mobile** sobre "Mobile Profiler" a secas: dice qué perfila sin atarse a una
...
`Generic/sample-android-profiler` → **`Generic/sample-mobile-profiler`**. Se eligió
kebab-case con el prefijo `sample-` para no romper la convención del scope `@generic/`.
...
`git remote set-url origin git@github.com:Generic/sample-mobile-profiler.git` para no
depender del redirect.

`~/.sample-profiler/` NO se toca: ya es agnóstico y migrarlo obligaría a mover las
